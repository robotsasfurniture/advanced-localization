...
def test_compare_gcc_and_cc():
    ...
    dibiase_srp_gcc = ConventionalSrp(fs, "2D", n_grid,
        mic_positions=mic_positions, room_dims=room_dims, interpolation=False,
        mode="gcc_phat_time")
    dibiase_srp_gcc_beta = ConventionalSrp(fs, "2D", n_grid,
        mic_positions=mic_positions, room_dims=room_dims, interpolation=False,
        mode="gcc_phat_beta")
    ...
    estimated_positions, srp_map, candidate_grid = dibiase_srp_gcc_beta.forward(signals)
    ...
    plot_uniform_cartesian_grid(candidate_grid, room_dims,
                                srp_map=srp_map, ax=axs[3],
                                mic_positions=mic_positions,
                                source_positions=source_position[np.newaxis, :])
    
    axs[0].set_title("Cross-correlation")
    axs[1].set_title("GCC-PHAT")
    axs[2].set_title("GCC-PHAT-BETA")
    axs[3].set_title("GCC-PHAT-FREQ")
